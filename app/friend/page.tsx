...
import { Search, UserPlus, Check, X, Users, User as UserIcon, UserMinus, Menu } from 'lucide-react';
...
export default function FriendPage() {
  ...
  const handleAddFriend = async (userObj: any) => {
    try {
      ...
      if (res.ok) {
        ...
        setSentRequestsList(prev => [...prev, userObj]);
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden relative">
      ...
      {toast && (
        <div className={`fixed top-8 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-2xl shadow-xl flex items-center gap-3 animate-in slide-in-from-top-5 fade-in duration-300 ${toast.type === 'success' ? 'bg-[#5D5FEF] text-white' : 'bg-rose-500 text-white'}`}>
      ...
      `}>
        <Sidebar activeTab="friends" onMobileItemClick={() => setIsSidebarOpen(false)} />
      </div>
